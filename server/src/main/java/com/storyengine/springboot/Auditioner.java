...
public class Auditioner implements Comparable{
    ...
    public void AddRolePermutations(Scene scene, ArrayList<Auditioner> auditioners, ArrayList<Requirement> requirementsList, Iterable<Status> statusList, Iterable<Flag> flagList, int time){
        ArrayList<Auditioner> cast = new ArrayList<Auditioner>();
        cast.add(this);
        AddRolePermutationsRecursive(scene, cast, auditioners, requirementsList, statusList, flagList, time);
    }

    private void AddRolePermutationsRecursive(Scene scene, ArrayList<Auditioner> cast, ArrayList<Auditioner> auditioners, ArrayList<Requirement> requirementsList, Iterable<Status> statusList, Iterable<Flag> flagList, int time){
        ...
        for(Auditioner nextAuditioner : auditioners){
            ...
            if(scene.AllRequirementsSatisfied(cast, requirementsList, statusList, flagList, time)){
